...
public class networkdiagram {
	 ...
	 static ArrayList<String> namesOrdered = new ArrayList<String>();
	 static ArrayList<Integer> durationOrdered = new ArrayList<Integer>();
...
while(check==false){
	...
	if(continuetest=='n'){
		...
		alphaNumeric();
		System.out.println("AN:"+namesOrdered);
		System.out.println(durationOrdered);
		createPaths();
...
//object.drawing(activityDependencies,activityName,activityDur,resultss);
...
//////////////// First Duration /////////////////////////////	
	...
	while(def==106576){																										// error checking
		aDur = JOptionPane.showInputDialog("What you doing entering a non-number????\nso disrespectful...\ndo it again....\nEnter the first node activty duration then hit ENTER");
...
public static void addActivity(){
	...
	while(bool==true){																										//error checking
		aDep = JOptionPane.showInputDialog("I told you no periods!\ndo it again...\nEnter the first node activty name then hit ENTER (your input may not contain a period('.')");
		bool = aDep.contains(".");
	}
...
public static void alphaNumeric(){
	...
	public static void handleDependencyError(){
		...
		for(int i=0; i<grandposition;i++){
			System.out.println("GRANDPOSITION "+grandposition);
			int x=0;
